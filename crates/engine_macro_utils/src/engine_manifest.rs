...
impl EngineManifest {
    /// Returns a global shared instance of the [`EngineManifest`] struct.
    pub fn shared() -> MappedRwLockReadGuard<'static, EngineManifest> {
        static MANIFESTS: RwLock<BTreeMap<PathBuf, EngineManifest>> = RwLock::new(BTreeMap::new());
    ...
    /// the [manifest](EngineManifest) by [name](str).
