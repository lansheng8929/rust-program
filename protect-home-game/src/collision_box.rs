...
impl CollisionBox {
    pub fn handle_wall_bounce(&self, transform: &Transform) -> (i32, i32) {
        ...
        if transform.position.0 <= 0.0 {
            bounce_x = -1;
        } else if transform.position.0 + self.width as f32 >= WINDOW_WIDTH as f32 {
            bounce_x = 1;
        }

        if transform.position.1 <= 0.0 {
            bounce_y = -1;
        } else if transform.position.1 + self.height as f32 >= WINDOW_HEIGHT as f32 {
            bounce_y = 1;
        }
    ...
    pub fn check_collision(
        ...
        let self_right = self_transform.position.0 + self.width as f32;
        let self_top = self_transform.position.1;
        let self_bottom = self_transform.position.1 + self.height as f32;

        let other_left = other_transform.position.0;
        let other_right = other_transform.position.0 + other.width as f32;
        let other_top = other_transform.position.1;
        let other_bottom = other_transform.position.1 + other.height as f32;
                    ...
                    *entity_id,
                    ...
                    (
                        (self_transform.position.0 + self_transform.velocity.0)
                            .min((WINDOW_WIDTH - collision_box.width) as f32),
                        (self_transform.position.1 + self_transform.velocity.1)
                            .min((WINDOW_HEIGHT - collision_box.height) as f32),
                    ),
                ));
            }
